...
<?php echo '<div '.$item_inner.'>';?>
   <?php if($item_thumbnail == 'yes' && (has_post_thumbnail() || !empty($thumbnail_default["id"]))):?>
        <div class="post-thumb <?php echo esc_attr($thumbnail_animation_default)?>">
            ...
            <?php  echo '<span class="date"><span class="day title16 font-semibold">'.date('d').'</span><span class="th title16">'.date('F').'</span></span>'; ?>
        </div>
    <?php endif?>   
    <div class="post-info text-center">
        <?php if($item_meta == 'yes') bzotech_display_metabox('grid-post',$item_meta_select,'','meta-post-style3');?>
        <?php if($item_title == 'yes'):?><h3 class="title20 post-title font-bold"><a class="color-title" href="<?php echo esc_url(get_the_permalink()) ?>"><?php the_title()?></a></h3><?php endif ?>
