...
if ($settings['nav_menu'] != '' && wp_get_nav_menu_items($settings['nav_menu']) !== false) {
	...
	if (isset($settings['bzotech_nav_menu_logo']['url']) && !empty($settings['bzotech_nav_menu_logo']['url'])) {
		$logo_mobile = '<a class="mobile-logo" href="' . esc_url(home_url('/')) . '">
							<img src="' . esc_url($settings['bzotech_nav_menu_logo']['url']) . '" alt="' . esc_attr__("logo mobile", 'bw-medxtore-v2') . '" >
						</a>';
	}
	...
	if ($settings['main_menu_style'] == "icon") {
		...
		if (!empty($settings['menu_icon_title_text'])) {
			$menu_icon_title_text = '<span class="e-toggle-style-icon-title">' . $settings['menu_icon_title_text'] . '</span>';
		}

		$position_content = 'position_content-' . $settings['menu_icon_position_content'] . ' sub-menu-display-' . $settings['sub_menu_icon_display_style'];
