...
switch ($content) {
	case 'category':
		...
		else echo wc_get_product_category_list( $product->get_id(), ', ', '<div class="posted_in"><label>' . _n( 'Category:', 'Categories:', count( $product->get_category_ids() ), 'bw-medxtore-v2' ) . '</label><div class="meta-item-list">', '</div></div>' );
		break;
	case 'tag':
	...
	else echo wc_get_product_tag_list( $product->get_id(), ', ', '<div class="tagged_as"><label>' . _n( 'Tag:', 'Tags:', count( $product->get_tag_ids() ), 'bw-medxtore-v2' ) . '</label><div class="meta-item-list">', '</div></div>' );
		break;
	case 'reviews':
	    ...
	    }else woocommerce_template_single_rating();
		break;
	case 'sold':
		if ( empty( $product ) ) echo '<div class="total-sold">Sold: <span class="count">0</span></div>';
	        ...
	        echo '<div class="total-sold">'.esc_html__('Sold: ','bw-medxtore-v2').'<span class="count">'.$total_sold.'</span></div>';
		}
	...
	case 'stock':
		if ( empty( $product ) ) echo '<div class="total-stock">stock: <span class="count">0</span></div>';
	        ...
	        echo '<div class="total-stock">'.esc_html__('Stock: ','bw-medxtore-v2').'<span class="count">'.$total_stock.'</span></div>';
		}
	...
	if ( empty( $product ) ) echo '<div class="sku_wrapper"><label>Sku: </label> <div class="meta-item-list"><span class="sku">N/A</span></div></div>'; 
	        ...
	        <div class="sku_wrapper"><label><?php echo esc_html($title); ?></label> <div class="meta-item-list"><span class="sku">
	            <?php if( $sku = $product->get_sku() )  echo esc_html($sku); else esc_html__( 'N/A', 'bw-medxtore-v2' ); ?>
