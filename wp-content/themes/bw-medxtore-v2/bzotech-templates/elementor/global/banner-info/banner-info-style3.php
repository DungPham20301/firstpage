			...
			if(!empty($list_point) and is_array($list_point)) { 
		        ...
		        foreach ($list_point as $key => $item ) {
		        	...
		        	if(!empty($item['id_product'])){
		        		...
		        		$point_info = '<h3 class="title_product title26 font-semibold title-color"><a href="'.get_permalink($item['id_product']).'">'.get_the_title($item['id_product']).'</a></h3>'.$product_point->get_price_html();
		        	}else{
