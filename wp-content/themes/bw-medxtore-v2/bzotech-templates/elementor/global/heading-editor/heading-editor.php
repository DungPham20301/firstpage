...
if($header_style == 'mouse-cursor'){
	...
	$wdata->add_render_attribute( 'heading', 'class', bzotech_implode($css_by_theme_title).' '.$class_title_shadow.' container-flex-e elbzotech-heading-global font-title text-css-e font-semibold  title48 color-title elbzotech-heading-global-'.$settings['header_style'] );
	...
	if(!empty( $icon_hedding['value'])){				
		if( $icon_hedding['library'] == 'svg')
			$title_icon = '<img class="elbzotech-heading-global__icon" alt="'.esc_attr__('svg','bw-medxtore-v2').'" src="'.esc_url($icon_hedding['value']['url']).'">';
