...
<div <?php echo apply_filters('bzotech_output_content', $wdata->get_render_attribute_string('wrapper'));?>>
	...
	if(!empty($icon_image['url'])){
		...
		echo '<div class="info-box-icon item-image-icon-e '.$class_icon_image_hover.' elementor-animation-'.$image_icon_animation_hover_css.'">';
			echo  '<a '.$wdata->get_render_attribute_string( 'link_icon' ).'>';
		...
		if( $icon['library'] == 'svg')
			echo '<div class="info-box-icon item-image-icon-e elementor-animation-'.$image_icon_animation_hover_css.'"><a '.$wdata->get_render_attribute_string( 'link_icon' ).'><img class="item-icon-e" alt="'.esc_attr__('svg','bw-medxtore-v2').'" src="'.esc_url($icon['value']['url']).'"></a></div>';
		else
		echo '<div class="info-box-icon elementor-animation-'.$image_icon_animation_hover_css.'"><a '.$wdata->get_render_attribute_string( 'link_icon' ).'><i class="item-icon-e title50  '.$icon['value'].' '.bzotech_implode($css_by_theme_text).'"></i></a></div>';
	} 
