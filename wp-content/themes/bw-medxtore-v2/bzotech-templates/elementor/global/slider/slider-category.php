                   ...
                   $link_html = $title_html =$desc_html = $image_html =$count_html ='';
                    if(!empty($item['category'])){
                        ...
                        if(!empty($cat->term_id)){
                            ...
                            $title_html = $cat->name;
                            if($cat->count !== 1) $count_label = esc_html__(' items','bw-kidxtore');
                            else $count_label = esc_html__(' item','bw-kidxtore');
                            $count_html = '<span class="cat-count hidden">'.$cat->count.$count_label.'</span>';
                    ...
                    if(!empty($item['template'])) 
                       ...
                       echo '<div class="cate-img overflow-hidden text-center"><a class="adv-thumb-link-cate adv-thumb-link" '.$link_html.'>'.$image_html.'</a></div>
                            <div class="info "><h2 class="info-title text-center font-medium title16 text-capitalize"><a class="color-title" '.$link_html.'>'.$title_html.'</a></h2>'.$count_html.'</div>';
                    }  
