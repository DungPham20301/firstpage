...
if(!function_exists('bzotech_shop_wrap_before')){
    function bzotech_shop_wrap_before(){
        ...
        ?>
        <div class="aaaaaa product-<?php echo esc_attr($view)?>-view <?php echo esc_attr($grid_type.' '.$gap_product)?> products-wrap js-content-wrap content-wrap-shop shop-<?php echo esc_attr($view)?>-product-item-<?php echo esc_attr($slug)?>" data-load="<?php echo esc_attr($data_ajax)?>">
        ...
        if($shop_style == 'load-more' && $max_page > 1){
            ...
            echo    '<input type="hidden" name="load-more-product-ajax-nonce" class="load-more-product-ajax-nonce" value="' . wp_create_nonce( 'load-more-product-ajax-nonce' ) . '" /><div class="btn-loadmore">
                        <a href="#" class="product-loadmore loadmore elbzotech-bt-default elbzotech-bt-medium" 
                            ...
                            '.esc_html__("Load more",'bw-medxtore-v2').'
                        </a>
                    </div>';
        }
...
if(!function_exists('bzotech_timer_countdown_product')){
    function bzotech_timer_countdown_product($class = ''){
        ...
        if($sales_price_from < $current_time[0] && $current_time[0] < $sales_price_to){
            ...
            echo '<div class="final-countdown flex-wrapper '.$class.' " data-day="'.esc_attr__('Days','bw-medxtore-v2').'" data-hrs="'.esc_attr__('Hours','bw-medxtore-v2').'" data-min="'.esc_attr__('Mins','bw-medxtore-v2').'" data-sec="'.esc_attr__('Secs','bw-medxtore-v2').'"  data-countdown="'.esc_attr($data_date).'"></div>';
...
if(!function_exists('bzotech_button_add_to_cart_ajax')){
    function bzotech_button_add_to_cart_ajax(){
        ...
        echo'<a data-quantity="" href="?add-to-cart='.esc_attr( $product->get_id() ).'" data-product_sku="'.$product->get_sku().'" data-variation_id = "" data-product_id="'.esc_attr( $product->get_id() ).'" class="elbzotech-bt-style2 ajax_add_to_cart"><span>'.esc_html__('Add To Cart','bw-medxtore-v2').'</span></a>';
        
    }
}
if(!function_exists('bzotech_add_to_cart_text_custom')){
    function bzotech_add_to_cart_text_custom(){
        return esc_html__('Buy Now','bw-medxtore-v2');
    }
}
/**
 * 
...
if(!function_exists('bzotech_product_quickview')){
    function bzotech_product_quickview($class = '',$icon = '',$echo = true) {
        ...
        <a title="'.esc_attr__("Quick View",'bw-medxtore-v2').'" data-product-id="'.get_the_id().'" href="'.esc_url(get_the_permalink()).'" class="product-quick-view quickview-link '.esc_attr($class).'">'.$icon.'</a>';
    ...
    function bzotech_product_label($echo = true, $style='') {
        ...
        if($product->is_on_sale()){
            ...
            if($from && ($product->get_type() == 'simple'||$product->get_type() == 'external')){
                ...
                if(!empty($percent)){
                    if($style == 'style2'){
                        $html .= apply_filters( 'woocommerce_sale_flash','<span class="sale">'.esc_html($percent).'% '.esc_html__('OFF','bw-medxtore-v2').'</span>');
                    }else $html .= apply_filters( 'woocommerce_sale_flash','<span class="sale">-'.esc_html($percent).'%<span class="hidden">'.esc_html__('On Sale','bw-medxtore-v2').'</span></span>');
                }
                
            }
        }
        if($uppsell < 0) $html .=   '<span class="new">'.esc_html__("new",'bw-medxtore-v2').'</span>';
        if($trending_product=='1') $html .=   '<span class="trending">'.esc_html__("Trending",'bw-medxtore-v2').'</span>';
...
if(!function_exists('bzotech_get_rating_html')){
    function bzotech_get_rating_html($echo = true, $count = true, $style = '',$star=''){
        ...
        if(!empty($count)){
            ...
            if($review_count==0) $review_count = esc_html__('No review','bw-medxtore-v2');
            else{
                $review_count = '('.$review_count.') <span>'.esc_html__('review','bw-medxtore-v2').'</span>';
            }
        }
...
if ( !function_exists( 'bzotech_catalog_ordering' ) ) {
    function bzotech_catalog_ordering($query,$set_orderby = '',$list_item = false,$add_class = '') {        
        ...
        $catalog_orderby_options = apply_filters( 'woocommerce_catalog_orderby', array(
            'menu_order' => esc_html__( 'Default sorting', 'bw-medxtore-v2' ),
            'popularity' => esc_html__( 'Sort by popularity', 'bw-medxtore-v2' ),
            'rating'     => esc_html__( 'Sort by average rating', 'bw-medxtore-v2' ),
            'date'       => esc_html__( 'Sort by latest', 'bw-medxtore-v2' ),
            'price'      => esc_html__( 'Sort by price: low to high', 'bw-medxtore-v2' ),
            'price-desc' => esc_html__( 'Sort by price: high to low', 'bw-medxtore-v2' )
        ) );
        ...
        if ( wc_get_loop_prop( 'is_search' ) ) {
            $catalog_orderby_options = array_merge( array( 'relevance' => esc_html__( 'Relevance', 'bw-medxtore-v2' ) ), $catalog_orderby_options );
    ...
    function bzotech_load_product_filter() {
        ...
        if(isset($pagination) && !empty($pagination)){?>
            <div class="products bzotech-row list-product-wrap js-content-main">
                ...
                else echo '<div class="filter-noresult-wrap"><div class="filter-noresult title18 text-center">'.esc_html__("No result found with current filter value.",'bw-medxtore-v2').'</div></div>';
                ?>
            </div>
            <?php
                        ...
                        <div class="bzotech-row">
                            ...
                            <div class="bzotech-col-md-7 bzotech-col-sm-6 bzotech-col-sx-6">
                                <div class="wrap-sticky-cart-price flex-wrapper align_items-center justify_content-flex-end">
                                    <?php if($product_type == 'simple' || $product_type == 'external') echo woocommerce_template_single_add_to_cart(); 
                                    else echo '<a data-title-hide="'.esc_html__('hidden options','bw-medxtore-v2').'" class="btn-sticky-addcart-option elbzotech-bt-default" href="#">'.esc_html__('view options','bw-medxtore-v2').'</a>';?>
...
if(!function_exists('bzotech_live_search')){
    function bzotech_live_search() {
        ...
        if( $query->have_posts() && !empty($key) && !empty($trim_key)){
            ...
            echo '<p class="text-no-results">'.esc_html__("No any results with this keyword.",'bw-medxtore-v2').'</p>';
        }
        wp_reset_postdata();
    }
    ...
    function bzotech_load_ajax_tool_panel() {
        ...
        $html = '<div class="dm-header">
                    ...
                    <div class="header-button">
                        <a target="_blank" class="elbzotech-bt-global-style2" href="'.esc_url($buy_link).'">'.esc_html__('Buy Now','bw-medxtore-v2').'</a>                  
                    </div>';
                                            
         $html .=   '<div class="header-description box-demo-home">
                        <h2>'.$title.'</h2>
                        <h4><span class="color2">'.esc_html__('Live preview','bw-medxtore-v2').'</span></h4>
                    </div>                                
        ...
        if (!empty($sale_price_dates_to) && $strtotime_date_to > $strtotime_date_from && $strtotime_date_to >= strtotime(date("Y/m/d"))) {
            ...
            if (!empty($stock)) {
                ...
                <div class="flashsale">
                    ...
                    <div class="sold-info"><?php echo sprintf(__('Sold: %s/%s', 'bw-medxtore-v2'), $buy,($stock + $buy)); ?></div>
    ...
    if(!function_exists('bzotech_add_custom_taxonomy_brand_woo')){
        function bzotech_add_custom_taxonomy_brand_woo (){
            bzotech_reg_taxonomy(
                ...
                'product',
                array(
                    'label' => esc_html__( 'Brand', 'bw-medxtore-v2' ),
                    'rewrite' => array( 'slug' => 'product-brands', 'bw-medxtore-v2' ),
                    'hierarchical' => true,
                    'query_var'  => true
                )
            );
    ...
    if(!function_exists('bzotech_metabox_register_brand_woo')){
        function bzotech_metabox_register_brand_woo($tag) { ?>        
            <div class="form-field">
                <label><?php esc_html_e('Logo brand','bw-medxtore-v2'); ?></label>
                <div class="wrap-metabox">
                    <div class="live-previews"></div>
                    <a class="button button-primary sv-button-remove"> <?php esc_html_e("Remove",'bw-medxtore-v2')?></a>
                    <a class="button button-primary sv-button-upload-id"><?php esc_html_e("Upload",'bw-medxtore-v2')?></a>
                    ...
                    <div class="wrap-metabox">
                        <div class="live-previews">
                            <?php 
                                ...
                                echo '<img alt="'.esc_attr__('image','bw-medxtore-v2').'" src="'.wp_get_attachment_url($image).'" />';
                            ?> 
                        </div>
                        <a class="button sv-button-remove"> <?php esc_html_e("Remove",'bw-medxtore-v2')?></a>
                        <a class="button button-primary sv-button-upload-id"><?php esc_html_e("Upload",'bw-medxtore-v2')?></a>
                            ...
                            <div class="attribute-item">
                                ...
                                <div class="attribute-item__value value">
                                    <?php
                                        ...
                                        echo end( $attribute_keys ) === $attribute_name ? wp_kses_post( apply_filters( 'woocommerce_reset_variations_link', '<a class="reset_variations hidden" href="#">' . esc_html__( 'Clear', 'bw-medxtore-v2' ) . '</a>' ) ) : '';
                                    ?>
                                </div>
                            </div>
                            <?php } ?>
