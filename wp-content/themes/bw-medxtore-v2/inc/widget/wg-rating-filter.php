...
if(!class_exists('Bzotech_Rating_Filter') && class_exists("woocommerce"))
{
    class Bzotech_Rating_Filter extends WP_Widget {
        ...
        function __construct() {
            ...
            parent::__construct( false, esc_html__('BZOTECH Rating Filter','bw-medxtore-v2'),
                array( 'description' => esc_html__( 'Display a list of star ratings to filter products in your store.', 'bw-medxtore-v2' ), ));

            $this->default=array(
                'title'=>esc_html__('Rating','bw-medxtore-v2'),
            );
        }
		...
		protected function get_filtered_product_count( $rating ) {
			...
			return absint( $wpdb->get_var( $wpdb->prepare($sql,'product','publish') ) ); // WPCS: unprepared SQL ok.
		}
