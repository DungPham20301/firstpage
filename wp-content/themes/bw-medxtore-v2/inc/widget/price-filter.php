...
if(class_exists('WC_Widget')){
	class Bzotech_Widget_Price_Filter extends WC_Widget {
		...
		public function __construct() {
			...
			$this->widget_description = esc_html__( 'Display a slider to filter products in your store by price.', 'bw-medxtore-v2' );
			$this->widget_id          = 'woocommerce_price_filter';
			$this->widget_name        = esc_html__( 'BZOTECH Filter By Price', 'bw-medxtore-v2' );
			$this->settings           = array(
				'title'  => array(
					'type'  => 'text',
					'std'   => esc_html__( 'Filter by price', 'bw-medxtore-v2' ),
			...
			echo '<form method="get" action="' . esc_url( $form_action ) . '">
					<div class="price_slider_wrapper">
						...
						<div class="price_slider_amount">
							<input type="text" id="min_price" name="min_price" value="' . esc_attr( $min_price ) . '" data-min="' . esc_attr( apply_filters( 'woocommerce_price_filter_widget_min_amount', $min ) ) . '" placeholder="' . esc_attr__( 'Min price', 'bw-medxtore-v2' ) . '" />
							<input type="text" id="max_price" name="max_price" value="' . esc_attr( $max_price ) . '" data-max="' . esc_attr( apply_filters( 'woocommerce_price_filter_widget_max_amount', $max ) ) . '" placeholder="' . esc_attr__( 'Max price', 'bw-medxtore-v2' ) . '" />
							...
							<button type="submit" class="elbzotech-bt-default">' . esc_html__( 'Filter', 'bw-medxtore-v2' ) . '</button>
