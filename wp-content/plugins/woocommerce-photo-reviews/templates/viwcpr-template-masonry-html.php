	...
	if ( $product ) {
            ...
            <div class="<?php echo esc_attr( $prefix_class ); ?>reviews-images-container">
				...
				if ( ! villatheme_is_url( $first_ele ) ) {
					...
					if ( $caption_enable ) {
						...
						if ( $is_video ) {
							printf( '<video class="%sreviews-images reviews-videos" data-original_src="%s" src="%s"  width="%s" height="%s" controls autoplay playsinline>%s</video>',
						...
						if ( $is_video ) {
							printf( '<div class="%sreviews-images-wrap-right"><video class="%sreviews-images reviews-videos" data-original_src="%s" src="%s" width="%s" height="%s" controls autoplay playsinline>%s</video></div>',
		...
		if ( $review_title_enable && ( $review_title = get_comment_meta( $comment->comment_ID, 'wcpr_review_title', true ) ) ) {
			$review_title = VI_WOOCOMMERCE_PHOTO_REVIEWS_Frontend_Frontend::wpml_translate_single_string(
				...
				'viwcpr-title-product-' . $comment->comment_post_ID . '-review-' . $comment->comment_ID,
				$review_title
			);
