...
//		}
	...
	public function settings_page() {
		...
		$orders_url=get_option( 'woocommerce_feature_custom_order_tables_enabled' ) === 'yes' && get_option( 'woocommerce_custom_orders_table_data_sync_enabled' ) === 'no' ?
                    ...
                    <div class="vi-ui segment">
                        <table class="form-table">
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="vi-ui toggle checkbox">
                                        <input type="hidden" name="reviews_order"
                                               value="<?php echo esc_attr( $reviews_order = $this->settings->get_params( 'reviews_order' ) ); ?>">
                                        <input type="checkbox"
                                               id="reviews_order" <?php checked( $reviews_order, '1' ) ?>>
			...
			if ( $screen && $screen->id == 'comment' ) {
				...
				if ( $review_title ) {
					...
					VI_WOOCOMMERCE_PHOTO_REVIEWS_Frontend_Frontend::wpml_register_single_string(
						...
						'viwcpr-title-product-' . wc_clean( wp_unslash( $_POST['comment_post_ID'] ) ) . '-review-' . $comment_id,
						$review_title
					);
